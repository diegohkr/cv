...
class SupabaseSearchService {
  ...
  /**
   * Búsqueda principal con integración MiniMax-M1 + Supabase
   */
  async search(query: string, limit: number = 10, useAI: boolean = true): Promise<SearchResponse> {
    const startTime = Date.now();

    try {
      console.log('🔍 Iniciando búsqueda híbrida MiniMax + Supabase:', query);

      // Paso 1: Análisis conversacional con MiniMax-M1
      let criteria: SearchCriteria;
      let conversationalResponse = '';
      
      // SIEMPRE usar análisis local para garantizar que funcione
      console.log('🔍 Usando análisis de búsqueda inteligente local (más confiable)');
      criteria = this.analyzeSearchQueryLocal(query);
      ...
      if (this.isSupabaseAvailable) {
        ...
        // Paso 3: Generar respuesta conversacional con MiniMax
        if (useAI && this.isMinimaxAvailable && result.companies.length > 0) {
          try {
            const minimaxCriteria: MinimaxSearchCriteria = {
              ...criteria,
              query,
              intent: 'search'
            };
            conversationalResponse = minimaxService.generateConversationalResponse(
              minimaxCriteria, 
              result.companies.map(r => r.company), 
              result.searchTime
            );
          } catch (error) {
            console.warn('⚠️ Error generando respuesta conversacional:', error);
          }
        }
